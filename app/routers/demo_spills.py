...
async def list_spills(page: int = Query(1, ge=1), page_size: int = Query(20, ge=1, le=100)):
    ...
    items = []
    for s in spills:
        items.append(DemoSpillListItem(
            spill_id=s["spill_id"],
            detected_at=datetime.fromisoformat(s["detected_at"]),
            centroid=CentroidSchema(lon=s["observation_longitude"], lat=s["observation_latitude"]),
            area_km2=s["area_km2"],
...
async def get_spill_detail(spill_id: str):
    ...
    if not s:
        raise HTTPException(status_code=404, detail="Spill not found in demo catalog")
    ...
    return DemoSpillDetail(
        ...
        estimated_release_time=datetime.fromisoformat(s["estimated_release_time"]),
...
async def get_spill_vessels(spill_id: str):
    s = SpillCatalogService.get_spill(spill_id)
    if not s:
        raise HTTPException(status_code=404, detail="Spill not found in demo catalog")
    ...
    if s["ranked_top_vessel"]:
        vessels.append(DemoSpillVessel(
            vessel_id=s["ranked_top_vessel"],
            is_mock=False,
            rank=1,
            score=s["ranked_top_score"]
        ))
...
@router.post("/{spill_id}/backtrack", response_model=DemoBacktrackResponse)
async def backtrack_spill(
    ...
    try:
        ...
        print(f"spill_id={spill_id} backtrack_runtime_seconds={elapsed:.3f}")
        ...
        if candidates:
            attribution_obj["top_score"] = candidates[0].get("total_score")
        ...
        raise HTTPException(status_code=500, detail="Internal server error during backtrack calculation") from exc
