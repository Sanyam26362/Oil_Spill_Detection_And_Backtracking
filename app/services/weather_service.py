...
class WeatherService:
    ...
    def get_velocities(
        ...
        if crosses_boundary:
            ...
            # The scalar reference implementation is used here
            # for the boundary case.
            #
            # This keeps boundary handling correct and robust.
            results = [
                self.get_velocity(
                    latitude=float(lat),
                    longitude=float(lon),
            ...
            return (
                np.asarray(
                    [r.wind_u for r in results],
                    dtype=float,
                ),
                np.asarray(
                    [r.wind_v for r in results],
                    dtype=float,
                ),
                np.asarray(
                    [r.current_u for r in results],
                    dtype=float,
                ),
                np.asarray(
                    [r.current_v for r in results],
                    dtype=float,
                ),
            )
